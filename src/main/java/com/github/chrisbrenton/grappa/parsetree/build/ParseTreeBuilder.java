...
public final class ParseTreeBuilder<V> extends ParseEventListener<V>
{
     ...
     * Our map of contexts.
     *
     * TODO: replace with an ArrayDeque; no need for indices since we have one
     * context per level anyway
     */
    ...
    public void matchSuccess(final MatchSuccessEvent<V> event){
        ...
        final int level = context.getLevel();
        /*
         * We have a success: remove the context at that level, and supply it
        ...
        /*
         * If we are back at the root, we're done: build the node.
         */
        if (level == 0) {
            rootNode = ctx.build();
            return;
        ...
        /*
         * If not, take the context at the immediate upper level and add this
    ...
    public void matchFailure(final MatchFailureEvent<V> event) {
        /*
         * If the match is a failure, remove the context at this level, if any.
         *
